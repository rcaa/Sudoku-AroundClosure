...
public privileged aspect UndoFeature {
	
	pointcut hook_createUndoItem(Gui cthis, JMenu optionsMenu) : execution(* Gui.hook_createUndoItem(..)) && this(cthis) && args(optionsMenu);
	
	before(Gui cthis, JMenu optionsMenu) : hook_createUndoItem(cthis, optionsMenu) {
		optionsMenu.add(cthis.createUndoMenuItem());
	}
	
	pointcut hook_undo(BoardManager cthis) : execution(* BoardManager.hook_undo(..)) && this(cthis);
	
	before(BoardManager cthis) : hook_undo(cthis) {
		cthis.undo();
	}
	
	pointcut hook_undo2(BoardManager bm) : execution(* SudokuGenerator.hook_undo2(..)) && args(bm);
